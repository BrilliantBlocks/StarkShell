...
@external
func test_constructor_on_erc721{
    ...
    local mint_hash;
    local mintBatch_hash;
    %{ 
        from starkware.starknet.public.abi import get_selector_from_name
        ids.mint_hash = get_selector_from_name("mint")
        ids.mintBatch_hash = get_selector_from_name("mintBatch")
    %}

    // Has mint and mintBatch
    let (selectors_len, selectors) = IDiamond.facetFunctionSelectors(diamond_address, universalMint_class_hash);
    assert_eq(selectors_len, 3);
    assert_eq(selectors[1], mint_hash);
    assert_eq(selectors[2], mintBatch_hash);
    return ();
}
